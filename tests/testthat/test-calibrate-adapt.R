test_that("PIT recalibration is a valid map that does not collapse tails", {
  train <- ref_simulate(200, seed = 18)
...
test_that("calibration by group uses the group map and pools unknown groups", {
  dat <- ref_simulate(400, site_shift = c(0, 2, 0, 0), seed = 21)
...
test_that("adapting to a site from the reference generator changes nothing", {
  dat <- ref_simulate(4000, site_log_scale = 0, seed = 23)
  train <- dat[dat$site != "B", ]
  local <- dat[dat$site == "B", ][1:150, ]
  held <- dat[dat$site == "B", ][151:900, ]
  fit <- ref_fit(simple_spec(), data = train, outcomes = "y")
  ad <- ref_adapt(fit, data = local, by = site, parameters = c("location", "scale"))
  expect_s3_class(ad, "ref_fit")
  expect_s3_class(ad$adaptation, "ref_adaptation")
...
test_that("adaptation recovers a scale ratio of 1.5 and a location shift", {
  dat <- ref_simulate(4000, site_shift = c(0, 1.2, 0, 0),
...
test_that("adaptation works for SHASH fits via normal scores", {
  dat <- ref_simulate(3200, kind = "shash", site_log_scale = c(0, log(1.5), 0, 0),
                       seed = 25)
  train <- dat[dat$site != "B", ]
  loc_b <- dat[dat$site == "B", ]
  fit <- ref_fit(simple_spec("shash"), data = train, outcomes = "y")
  ad <- ref_adapt(fit, data = loc_b[1:300, ], by = site, parameters = c("location", "scale"))
...
test_that("adapted predict honours extrapolation, ids, and total uncertainty", {
  dat <- ref_simulate(300, seed = 26)
  dat$subject <- sprintf("s%03d", seq_len(nrow(dat)))
  fit <- ref_fit(simple_spec(), data = dat[1:220, ], outcomes = "y", id = subject)
  ad <- ref_adapt(fit, data = dat[221:260, ], parameters = "location")
  new <- dat[261:270, ]
  new$age[1] <- 140
...
test_that("total-uncertainty adaptation propagates the offset SE exactly for a draw mixture", {
  dat <- ref_simulate(260, kind = "gaussian", scale = "age", seed = 71)
  spec <- ref_spec(family = ref_gaussian(), location = ~ s(age, k = 5) + sex,
                    scale = ~ s(age, k = 4))
  fit <- ref_fit(spec, data = dat[1:200, ], outcomes = "y")
  ad <- ref_adapt(fit, data = dat[201:240, ], parameters = "location")
...
test_that("joint location+scale adaptation is not biased by feedback between the offsets", {
  ...
  ref <- ref_simulate(1000, seed = 100)
  fit <- ref_fit(ref_spec(ref_gaussian(), ~ s(age, k = 5) + sex), data = ref, outcomes = "y")
  one <- function(seed, n_local, lp = 10, sp = 25) {
    site <- ref_simulate(n_local + 1000, site_shift = 6, site_log_scale = 0.2, seed = seed)
    loc <- site[seq_len(n_local), ]
    held <- site[-seq_len(n_local), ]
    ad <- ref_adapt(fit, loc, parameters = c("location", "scale"),
                     location_prior_n = lp, scale_prior_n = sp)
    off <- ad$adaptation$offsets$y$.all
    both <- ref_adapt(fit, loc, parameters = "location", location_prior_n = lp)
