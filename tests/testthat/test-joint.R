...
test_that("joint centiles are calibrated irrespective of how many outcomes are observed", {
  ref_dat <- norm_simulate(600, seed = 27)
  new_dat <- norm_simulate(3000, seed = 28)
  fit <- norm_fit(simple_spec(), data = ref_dat, outcomes = starts_with("marker_"))
  cf <- norm_crossfit(simple_spec(), data = ref_dat, outcomes = starts_with("marker_"), folds = 4)
  jt <- norm_joint(cf)
