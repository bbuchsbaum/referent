test_that("held-out Gaussian coverage stays near nominal", {
  set.seed(50)
  train <- ref_simulate(280, seed = 50)
  test <- ref_simulate(220, seed = 51)
...
test_that("in-sample Z scores are over-shrunk relative to cross-fit", {
  ...
  cf <- ref_crossfit(simple_spec(), data = dat, outcomes = "y",
...
test_that("dynamics refuse unusual-change labels without repeats", {
  ...
  tr <- ref_transition(dyn, data = dat2, id = participant_id, time = age)
...
test_that("SHASH beats Gaussian on strongly skewed held-out data", {
  ...
  fit_g <- ref_fit(simple_spec(scale = TRUE), data = train, outcomes = "y")
  fit_s <- ref_fit(simple_spec("shash", scale = TRUE), data = train, outcomes = "y")
  skip_if(
    identical(fit_s$models$y$status, "nonconverged"),
    "SHASH fit did not converge"
  )

  a_g <- ref_assess(fit_g, newdata = test)
  a_s <- ref_assess(fit_s, newdata = test)
...
test_that("Gaussian remains competitive when the truth is Gaussian", {
  skip_on_cran()
  train <- ref_simulate(240, kind = "gaussian", scale = "age", seed = 63)
  test <- ref_simulate(180, kind = "gaussian", scale = "age", seed = 64)
  fit_g <- ref_fit(simple_spec(scale = TRUE), data = train, outcomes = "y")
  fit_s <- ref_fit(simple_spec("shash", scale = TRUE), data = train, outcomes = "y")
  skip_if(
    identical(fit_s$models$y$status, "nonconverged"),
